...
export function ShowProfile({ setShow }) {
    const [store] = useContext(GlobalContext)
    const { profileSel } = store
    return (
        <div className="relative max-w-sm mx-auto overflow-hidden bg-white rounded-lg shadow-lg dark:bg-gray-800">
            <div className="cursor-pointer absolute top-0 right-0 mt-4 mr-5 text-gray-400 hover:text-gray-600 dark:hover:text-gray-500 transition duration-150 ease-in-out" onClick={() => setShow(false)} >
            ...
            <img className="object-cover object-top w-full h-60" src={profileSel.profile.avatar} alt="avatar" />
            ...
            <div className="px-6 py-4">
                <h1 className="text-xl font-semibold text-gray-800 dark:text-white">{profileSel.userProfile.name}</h1>
                ...
                <div className="flex items-center mt-4 text-gray-700 dark:text-gray-200">
                    ...
                    <h1 className="px-2 text-sm">{profileSel.userProfile.email}</h1>
                </div>
            </div>
        </div>
    )
